...
mod tests {
    ...
    #[test]
    fn libkrun_requires_guest_image() {
        let dir = tempdir().unwrap();
        let spec = VmInstanceSpec {
            sandbox_id: "demo".to_string(),
            ...
            ready_file: dir.path().join("work/ready.json"),
            parent_pid: None,
            vmm_kind: VmmKind::Libkrun,
            boot_args: None,
            firecracker_api_socket: dir.path().join("work/fc.sock"),
            vsock_uds_path: dir.path().join("work/guest.vsock"),
            agent_socket_path: dir.path().join("work/agent.sock"),
            ready_socket_path: dir.path().join("work/ready.sock"),
            guest_cid: 3,
            agent_vsock_port: 26_950,
